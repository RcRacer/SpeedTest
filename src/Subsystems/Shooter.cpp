...
Shooter::Shooter() : Subsystem("Shooter") {
    ...
    shooterMotorR->SetControlMode(CANSpeedController::kSpeed);
    // Quadrature EncoderCPR=1024
    shooterMotorL->SetFeedbackDevice(CANTalon::QuadEncoder);
    shooterMotorL->ConfigEncoderCodesPerRev(1440);
    shooterMotorL->SetSensorDirection(true);
    shooterMotorR->SetFeedbackDevice(CANTalon::QuadEncoder);
    shooterMotorR->ConfigEncoderCodesPerRev(1440);
    shooterMotorR->SetSensorDirection(false);
}
...
void Shooter::SetSpeed(int speed) {
	...
	double ramprate = 48;	/* volts per second, =>0% to 100% in 250ms, 48 */
